...
def load_checkpoint(
    ...
    incompatible_keys = model.load_state_dict(state_dict, strict=False)#strict)
    return incompatible_keys

# Defining the model layers to remain unfrozen: freezing reduces computational resource requirements, improves transfer learning efficiency, but also limits the extent of learning specific to the new task
# Freeze all layers besides last two & multihead attention
...
def create_model(
    ...
    # CHANGED: freeze layers
    """
    trainable_params_count = 0

    for name, param in model.named_parameters():
        if name in modules_to_freeze_resnet:
            param.requires_grad = False
            print(f"Froze parameter: {name}")
        else:
            param.requires_grad = True
            trainable_params_count += param.numel()  # Count all elements in the parameter
...
modules_to_not_freeze_vit = [
    "cross_attention_module.multihead_attn.in_proj_weight",
    "cross_attention_module.multihead_attn.in_proj_bias",
    "cross_attention_module.multihead_attn.out_proj.weight",
    "cross_attention_module.multihead_attn.out_proj.bias",
    
    "cross_attention_module.layernorm.weight",
    "cross_attention_module.layernorm.bias",
    "cross_attention_module.ffn.0.weight",
    "cross_attention_module.ffn.0.bias",
    "cross_attention_module.ffn.2.weight",
    "cross_attention_module.ffn.2.bias",
